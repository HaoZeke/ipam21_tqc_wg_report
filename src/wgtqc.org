...
Multiple challenges exist to enhance the performance of tensor contractions.
\emph{High computing power needed to compute these contractions.}
A tensor contraction could easily goes to very high computation complexity. 
Give two $N^{th}$-order cubic tensors to be contracted on $M$ dimensions with dimension size $I$ needs $\mathcal{O}(I^{2N-M})$ floating-point operations (Flops).
...
\emph{Huge space is required to run these contractions in memory.}

\emph{Parallelized on distributed memory systems meet the scalability issue.}

\emph{Sparsity, adopted in tensor contractions to reduce computing requirements, needs good algorithms and data structures to achieve high performance.}
